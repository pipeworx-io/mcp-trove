 ...
 * Trove MCP.
 *
 * Trove (National Library of Australia) — 6B+ items: digitised newspapers, books, images, maps, archives & more. BYO key: this pack requires an API key you supply yourself via _apiKey on every call. Get one free at trove.nla.gov.au (account -> Create an API key). Pipeworx does not front a key for this source.
 */
...
async function pwFetch(url: string | URL, init?: RequestInit): Promise<Response> {
  return fetchWithTimeout(url, init ?? {}, 'Trove');
}
...
const tools: McpToolExport['tools'] = [
  {
    name: 'search',
    description: 'Search the Trove collection by keyword. Returns matching items with ids (pass an id to work), titles, creators/sources, dates and links. Requires your own free Trove API key, passed as _apiKey.',
    inputSchema: { type: 'object', properties: {
      query: { type: 'string', description: 'Keyword(s).' },
      limit: { type: 'number', description: 'Max results (1-100, default 20).' },
      page: { type: 'number', description: 'Page (1-based, default 1).' },
      _apiKey: { type: 'string', description: 'Your own Trove API key (BYO — Pipeworx does not supply one). Free at trove.nla.gov.au: create an account, then Create an API key.' },
    }, required: ['query', '_apiKey'] },
  },
  {
    name: 'work',
    description: 'Fetch full details for one Trove item by id — a Trove work id (from search). Requires your own free Trove API key, passed as _apiKey.',
    inputSchema: { type: 'object', properties: {
...
async function callTool(name: string, args: Record<string, unknown>): Promise<unknown> {
  ...
  delete args._apiKey;
  if (!key) throw new Error('Trove requires an API key. Pass your own key via _apiKey — Pipeworx does not front one for this source. Get one free at trove.nla.gov.au (create an account, then Create an API key).');
  switch (name) {
    case 'search': {
